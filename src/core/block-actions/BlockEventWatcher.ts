...
import type { BlockActionEngine } from "@/core/block-actions/BlockActionEngine";
import type { BlockEvent } from "@/core/block-actions/BlockActionTypes";
import type { BlockActionSettings } from "@/core/settings/PluginSettings";
...
export class BlockEventWatcher {
  ...
  notifyBlockMoved(event: Omit<BlockEvent, "type">): void {
    ...
    void this.options.engine.handleBlockEvent(movedEvent);
  }
  ...
  notifyBlockCollapsed(blockId: string, collapsed: boolean): void {
    ...
    void this.options.engine.handleBlockEvent(event);
  }
