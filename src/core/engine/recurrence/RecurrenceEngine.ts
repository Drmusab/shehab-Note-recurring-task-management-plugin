...
export class RecurrenceEngine implements IRecurrenceEngine {
  ...
  explain(task: Task, ref: Date): RecurrenceExplanation {
    ...
    try {
      ...
      return {
        taskId: task.id,
        referenceDate: ref.toISOString(),
        rule: task.frequency.rruleString || '',
        mode: 'fixed',
        resultDate: null,
        evaluationSteps: [{
          step: 1,
  ...
  private getRRule(task: Task): RRule {
    ...
    const dtstart = task.frequency?.dtstart 
      ? new Date(task.frequency.dtstart)
      : new Date(task.dueAt);
  ...
  private getBaseDate(task: Task, ref: Date): Date {
    ...
    if (whenDone) {
      // For whenDone mode, calculate from completion/reference date
      return ref;
    }
